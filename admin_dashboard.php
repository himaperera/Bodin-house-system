    ...
    <aside class="sidebar">
      <nav class="sidebar-nav">
        ...
        <a href="admin_dashboard.php?tab=bookings" class="nav-item">
          <svg viewBox="0 0 24 24">
            <path
              d="M17 12h-5v5h5v-5zM16 1v2H8V1H6v2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2h-1V1h-2zm3 18H5V8h14v11z" />
          </svg>
          Bookings
        </a>
    ...
    <main class="main-content">
      <div class="page-header-row">
        <div>
          ...
          <div class="page-subtitle">Here's what's happening at your boarding house today —
            <?= date('l, F j, Y') ?>
          </div>
        </div>
      ...
      <!-- PENDING AD APPROVALS SECTION -->
      <div class="card" style="margin-bottom:22px; border-left: 4px solid var(--amber);">
        <div class="card-header">
          <div class="card-title"><i class="fa-solid fa-clock-rotate-left"
              style="color:var(--amber); margin-right:8px;"></i> Pending Ad Approvals <span class="pill pill-warning"
        ...
        <div class="card-body">
          <?php if (empty($pending_ads)): ?>
            <p style="color: var(--muted); margin: 0; padding: 10px 0;">No pending ads to review.</p>
          <?php else: ?>
            <div style="display: flex; flex-direction: column; gap: 12px; margin-top: 10px;">
              <?php foreach ($pending_ads as $ad): ?>
                <div
                  style="background: var(--offwhite); border: 1px solid rgba(251, 191, 36, 0.4); border-radius: 10px; padding: 15px 20px; display: flex; justify-content: space-between; align-items: center; transition: 0.3s;">
                  <div>
                    <h3 style="margin: 0 0 6px 0; font-size: 16px; color: var(--text);">Room
                      <?= htmlspecialchars($ad['room_number']) ?> -
                      <?= htmlspecialchars($ad['room_type']) ?>
                    </h3>
                    <p style="margin: 0; font-size: 13px; color: var(--muted);">
                      Submitted by: <strong style="color: var(--text);">
                        <?= htmlspecialchars($ad['owner_name']) ?>
                      </strong> &nbsp;|&nbsp; Price: <strong>Rs.
                        <?= number_format($ad['price']) ?>
                      </strong>
                    </p>
                  </div>
                  <a href="review_ad.php?id=<?= $ad['id'] ?>" class="btn btn-sm"
                    style="background: var(--amber); color: #000; font-weight: bold; border: none;">Review Ad ➔</a>
                </div>
              <?php endforeach; ?>
            </div>
          <?php endif; ?>
      ...
      <div class="grid-2" style="margin-bottom:22px;">
        <div class="card">
          <div class="card-header">
            ...
            <span
              class="pill <?= $occupancy >= 75 ? 'pill-success' : ($occupancy >= 50 ? 'pill-warning' : 'pill-danger') ?>">
              <?= $occupancy ?>% full
            </span>
          </div>
          <div class="card-body">
            ...
            foreach ($floors as $f => $p):
              $c = $p >= 75 ? 'var(--red)' : ($p >= 50 ? 'var(--amber)' : 'var(--green)'); ?>
              <div class="occ-row">
                <div class="occ-label">
                  <?= $f ?>
                </div>
                <div class="occ-bg">
                  <div class="occ-fill" style="width:<?= $p ?>%;background:<?= $c ?>"></div>
                </div>
                <div class="occ-pct">
                  <?= $p ?>%
                </div>
        ...
        <div class="card">
          ...
          <div class="card-body" style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
            ...
            foreach ($actions as $a): ?>
              <a href="<?= $a[0] ?>"
                style="background:var(--offwhite);border-radius:10px;padding:16px;text-decoration:none;display:flex;flex-direction:column;align-items:flex-start;gap:8px;border:1.5px solid transparent;transition:all .2s;"
                onmouseover="this.style.borderColor='var(--red)';this.style.background='#fff'"
                onmouseout="this.style.borderColor='transparent';this.style.background='var(--offwhite)'">
                <div
          ...
          <table class="data-table">
            ...
            <tbody>
              <?php if (empty($recentBookings)): ?>
                <tr>
                  <td colspan="9" style="text-align:center;color:var(--muted);padding:30px;">No bookings yet. <a
                      href="book.php" style="color:var(--red)">Create the first one</a></td>
                </tr>
              <?php else:
                foreach ($recentBookings as $i => $b):
                  ...
                  <tr>
                    ...
                    <td style="font-weight:700;color:var(--red);">Rs.
                    ...
                    <td>
                      <div style="display:flex;gap:6px;">
                        ...
                        <?php if ($b['status'] === 'pending'): ?>
                          <a href="update_booking.php?id=<?= $b['id'] ?>&action=confirm" class="btn btn-sm"
                            style="background:var(--green-light);color:#1b5e20;border:1px solid #a5d6a7;">Confirm</a>
                        <?php endif; ?>
                      </div>
                    </td>
                  </tr>
        ...
        <div class="table-wrap">
          <table class="data-table">
            ...
            <tbody>
              <?php if (empty($rooms)): ?>
                <tr>
                  <td colspan="8" style="text-align:center;color:var(--muted);padding:30px;">No rooms added yet. <a
                      href="add_room.php" style="color:var(--red)">Add your first room</a></td>
                </tr>
              <?php else:
                foreach ($rooms as $r):
                  ...
                  ?>
                  <tr>
                    <td style="font-weight:700;">Rm
                      <?= htmlspecialchars($r['room_number']) ?>
                    </td>
                    <td style="color:var(--muted);">
                      <?= htmlspecialchars($r['room_type']) ?>
                    </td>
                    <td style="color:var(--muted);">Floor
                      <?= htmlspecialchars($r['floor'] ?? 1) ?>
                    </td>
                    <td style="font-weight:700;color:var(--red);">Rs.
                      <?= number_format($r['price']) ?>
                    </td>
                    <td style="font-size:12px;color:var(--muted);">
                      <?= htmlspecialchars($r['amenities'] ?? '—') ?>
                    </td>
                    <td>
                      <?= $r['tenant_name'] ? '<div style="display:flex;align-items:center;gap:6px;"><div class="avatar" style="width:26px;height:26px;font-size:10px;">' . strtoupper(substr($r['tenant_name'], 0, 2)) . '</div>' . htmlspecialchars($r['tenant_name']) . '</div>' : '<span style="color:var(--muted);">—</span>' ?>
                    </td>
                    <td><span class="pill <?= $sc ?>">
                        <?= ucfirst($r['status']) ?>
                      </span></td>
                    <td>
                      <div style="display:flex;gap:6px;">
                        <a href="edit_room.php?id=<?= $r['id'] ?>" class="btn btn-ghost btn-sm">Edit</a>
                        <a href="delete_room.php?id=<?= $r['id'] ?>" class="btn btn-danger btn-sm"
